...
public class PostServiceImpl extends ServiceImpl<PostMapper, Post> implements PostService {
    ...
    public Page<Post> searchFromEs(PostQueryRequest postQueryRequest) {
        ...
        highlightFields.add(descriptionField);
        highlightFields.add(contentField);
        ...
        NativeSearchQuery searchQuery = new NativeSearchQueryBuilder().withQuery(boolQueryBuilder)
                .withHighlightFields(highlightFields.toArray(new HighlightBuilder.Field[0]))
...
//                        resourceList.add(idPostMap.get(postId).get(0));
                        ...
                        SearchHit<PostEsDTO> hit = searchHitList.stream().filter(sh -> sh.getContent().getId().equals(postId)).findFirst().orElse(null);
                        if (hit != null) {
                            // 获取高亮结果
                            Map<String, List<String>> highlightFieldsMap = hit.getHighlightFields();
                            if (highlightFieldsMap.containsKey("title")) {
                                post.setTitle(highlightFieldsMap.get("title").get(0));
                            }
//                            if (highlightFieldsMap.containsKey("description")) {
//                                post.setDescription(highlightFieldsMap.get("description").get(0));
//                            }
                            if (highlightFieldsMap.containsKey("content")) {
                                post.setContent(highlightFieldsMap.get("content").get(0));
                            }
                        }
